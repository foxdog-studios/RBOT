...
class SignedDistanceTransform2D
{
  public:
    /**
     *  Initializes the an instance with a specified maximum distance at wich
     * the clostest contour points for every pixel are still computed.
     *
     *  @param  maxDist The maximal absolute distance at which the closest
     * contour points are being comuted.
     */
    ...
    /**
     *  Computes the 2D Euclidean signed distance transform of a given input
     * image as well as the coordinates of the clostest contour location for
     * every pixel with CPU multi-threading.
     *
     *  @param  src The input image of which the distance transform shall be
     * computed (single channel, float of uchar).
     *  @param  sdt The output 2D Euclidean signed distance transform of src.
     *  @param  xyPos The per pixel 2D coordinates of the closest contour points
     * (two channel, integer).
     *  @param  threads The number of threads to be used for parallelization.
     *  @param  key In case of a uchar input image that is not binary, the value
     * specidfies the intensitiy to be considered foregorund (default = 0, i.e.
     * anything not equal to 0 is considered foreground).
     */
    void computeTransform(const cv::Mat& src,
                          cv::Mat& sdt,
                          cv::Mat& xyPos,
                          int threads,
                          uchar key = 0);
    ...
    /**
     *  Computes the first order derivatives of a given 2D Euclidean signed
     * distance level-set in x- and y- direction at each pixel using central
     * differences with CPU multi-threading.
     *
     *  @param  sdt The input 2D Euclidean signed distance transform of which
     * the derivatives shall be computed (single channel, float).
     *  @param  dX The output derivatives in x-direction (single channel,
     * float).
     *  @param  dY The output derivatives in y-direction (single channel,
     * float).
     *  @param  threads The number of threads to be used for parallelization.
     */
    void computeDerivatives(const cv::Mat& sdt,
                            cv::Mat& dX,
                            cv::Mat& dY,
                            int threads);
 ...
 *  computations. Within the corresponding for loop, for every row in a binary
 * input image the per pixel 1D signed distance transform is computed. Here,
 * also the x locations of the closest contour points per pixel are calculated.
 */
template <class type>
...
/**
 *  This class extends the OpenCV ParallelLoopBody for efficiently parallelized
 *  computations. Within the corresponding for loop, for every row in a input
 * image the per pixel 1D signed distance transform is computed based on a given
 * key value that specifies the intensitiy of the foreground region. Here, also
 * the x locations of the closest contour points per pixel are calculated.
 */
class Parallel_For_distanceTransformRowsWithKey : public cv::ParallelLoopBody
{
  ...
  public:
    Parallel_For_distanceTransformRowsWithKey(const cv::Mat& src,
                                              uchar key,
                                              cv::Mat& dd,
                                              cv::Mat& xPos,
                                              int* v,
...
/**
 *  This class extends the OpenCV ParallelLoopBody for efficiently parallelized
 *  computations. Within the corresponding for loop, the per pixel 2D signed
 * distance transform is computed for every column based on the previously
 * transformed rows. Here, also the 2D locations of the closest contour points
 * per pixel are calculated.
 */
            ...
            else
            {
                ...
                do
                {
                    ...
                    for (;;)
                    {
                        ...
                        {
                            int py =
                                (i < zeroPosY) ? zeroPosY - !bg : zeroPosY - bg;
...
/**
 *  This class extends the OpenCV ParallelLoopBody for efficiently parallelized
 *  computations. Within the corresponding for loop, for each pixel the central
 * differences in x-direction are computed from a given 2D Euclidean singed
 * distance transform.
 */
template <class type>
class Parallel_For_distanceTransformDX : public cv::ParallelLoopBody
{
  ...
  public:
    Parallel_For_distanceTransformDX(const cv::Mat& sdt,
                                     cv::Mat& dX,
                                     int threads)
    {
        _sdt = sdt;
    ...
    virtual void operator()(const cv::Range& r) const
    {
        ...
        for (int y = r.start * range; y < yEnd; y++)
        {
            ...
            {
                dX[row_idx + x] =
                    (sdt[row_idx + x + 1] - sdt[row_idx + x - 1]) / div;
                x++;
            }
        }
    }
 ...
 *  This class extends the OpenCV ParallelLoopBody for efficiently parallelized
 *  computations. Within the corresponding for loop, for each pixel the central
 * differences in y-direction are computed from a given 2D Euclidean singed
 * distance transform.
 */
template <class type>
class Parallel_For_distanceTransformDY : public cv::ParallelLoopBody
{
  ...
  public:
    Parallel_For_distanceTransformDY(const cv::Mat& sdt,
                                     cv::Mat& dY,
                                     int threads)
    {
        _sdt = sdt;
    ...
    virtual void operator()(const cv::Range& r) const
    {
        ...
        for (int y = yStart; y < yEnd; y++)
        {
            ...
            {
                dY[row_idx + x] =
                    (sdt[row_idx_next + x] - sdt[row_idx_prev + x]) / div;
                x++;
            }
        }
    }
