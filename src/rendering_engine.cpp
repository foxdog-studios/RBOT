...
RenderingEngine::RenderingEngine()
{
    ...
    this->surface.setFormat(glFormat);
    this->surface.create();
    ...
    projectionMatrix =
        Transformations::perspectiveMatrix(40, 4.0f / 3.0f, 0.1, 1000.0);
...
void RenderingEngine::init(const Matx33f& K,
                           int width,
                           int height,
                           float zNear,
                           float zFar,
                           int numLevels)
{
    ...
    projectionMatrix =
        Transformations::perspectiveMatrix(K, width, height, zNear, zFar, true);
...
bool RenderingEngine::initRenderingBuffers()
{
    ...
    glFramebufferTexture2D(
        GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthTextureID, 0);
...
bool RenderingEngine::initShaderProgram(QOpenGLShaderProgram* program,
                                        QString shaderName)
{
    if (!program->addShaderFromSourceFile(QOpenGLShader::Vertex,
                                          shaderFolder + shaderName +
                                              "_vertex_shader.glsl"))
    {
...
void RenderingEngine::renderSilhouette(Model* model,
                                       GLenum polyonMode,
                                       bool invertDepth,
                                       float r,
                                       float g,
...
void RenderingEngine::renderShaded(
    Model* model, GLenum polyonMode, float r, float g, float b, bool drawAll)
{
    vector<Model*> models;
    models.push_back(model);
...
void RenderingEngine::renderNormals(Model* model,
                                    GLenum polyonMode,
                                    bool drawAll)
{
    vector<Model*> models;
...
void RenderingEngine::renderSilhouette(vector<Model*> models,
                                       GLenum polyonMode,
                                       bool invertDepth,
                                       const std::vector<cv::Point3f>& colors,
                                       bool drawAll)
{
    ...
    for (int i = 0; i < models.size(); i++)
    {
        ...
        if (model->isInitialized() || drawAll)
        {
            ...
            Matx44f modelViewProjectionMatrix =
                projectionMatrix * modelViewMatrix;

            silhouetteShaderProgram->bind();
            silhouetteShaderProgram->setUniformValue(
            ...
            {
                color =
                    Point3f((float)(model->getModelID()) / 255.0f, 0.0f, 0.0f);
            }
            silhouetteShaderProgram->setUniformValue(
                "uColor", QVector3D(color.x, color.y, color.z));
...
void RenderingEngine::renderShaded(vector<Model*> models,
                                   GLenum polyonMode,
                                   const std::vector<cv::Point3f>& colors,
                                   bool drawAll)
{
    ...
    for (int i = 0; i < models.size(); i++)
    {
        ...
        if (model->isInitialized() || drawAll)
        {
            ...
            Matx33f normalMatrix =
                modelViewMatrix.get_minor<3, 3>(0, 0).inv().t();

            Matx44f modelViewProjectionMatrix =
                projectionMatrix * modelViewMatrix;

            phongblinnShaderProgram->bind();
            phongblinnShaderProgram->setUniformValue(
                "uMVMatrix", QMatrix4x4(modelViewMatrix.val));
            phongblinnShaderProgram->setUniformValue(
                "uMVPMatrix", QMatrix4x4(modelViewProjectionMatrix.val));
            phongblinnShaderProgram->setUniformValue(
                "uNormalMatrix", QMatrix3x3(normalMatrix.val));
            phongblinnShaderProgram->setUniformValue(
                "uLightPosition1", QVector3D(0.1, 0.1, -0.02));
            phongblinnShaderProgram->setUniformValue(
                "uLightPosition2", QVector3D(-0.1, 0.1, -0.02));
            phongblinnShaderProgram->setUniformValue("uLightPosition3",
...
void RenderingEngine::renderNormals(vector<Model*> models,
                                    GLenum polyonMode,
                                    bool drawAll)
{
    glViewport(0, 0, width, height);
    ...
    for (int i = 0; i < models.size(); i++)
    {
        ...
        if (model->isInitialized() || drawAll)
        {
            ...
            normalsShaderProgram->setUniformValue(
                "uMVMatrix", QMatrix4x4(modelViewMatrix.val));
            normalsShaderProgram->setUniformValue(
                "uMVPMatrix", QMatrix4x4(modelViewProjectionMatrix.val));
            normalsShaderProgram->setUniformValue("uNormalMatrix",
...
void RenderingEngine::projectBoundingBox(Model* model,
                                         ...
                                         cv::Rect& boundingRect)
{
    ...
    {
        Vec4f p = calibrationMatrices[currentLevel] * pose * normalization *
                  points3D[i];
...
Mat RenderingEngine::downloadFrame(RenderingEngine::FrameType type)
{
    ...
    switch (type)
    {
        case MASK:
            ...
            glReadPixels(
                0, 0, res.cols, res.rows, GL_RED, GL_UNSIGNED_BYTE, res.data);
            break;
        case RGB:
            res = Mat(height, width, CV_8UC3);
            glReadPixels(
                0, 0, res.cols, res.rows, GL_RGB, GL_UNSIGNED_BYTE, res.data);
            break;
        case RGB_32F:
